...
def scan_code(code, language, vulnera):
    """
    Scan code with semgrep
    """
    language_class = vulnera.computer.terminal.get_language(language)

    temp_file = create_temporary_file(
        code, language_class.file_extension, verbose=vulnera.verbose
    )
    ...
    try:
        ...
        if scan.returncode == 0:
            ...
            print(
                f"  {'Code Scanner: ' if vulnera.safe_mode == 'auto' else ''}No issues were found in this {language_name} code."
            )
            print("")
    ...
    cleanup_temporary_file(temp_file, verbose=vulnera.verbose)
