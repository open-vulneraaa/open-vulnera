"""
Sends anonymous telemetry to posthog. This helps us know how people are using Vulnera / what needs our focus.

Disable anonymous telemetry by execute one of below:
1. Running `vulnera --disable_telemetry` in command line.
2. Executing `vulnera.disable_telemetry = True` in Python.
...
def get_or_create_uuid():
    try:
        uuid_file_path = os.path.join(
            os.path.expanduser("~"), ".cache", "open-vulnera", "telemetry_user_id"
        )
        os.makedirs(
...
def send_telemetry(event_name, properties=None):
    if properties is None:
        properties = {}
    properties["ov_version"] = version("open-vulnera")
    try:
