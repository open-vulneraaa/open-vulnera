...
def get_ov_version():
    try:
        ov_version_cmd = subprocess.check_output(
            ["vulnera", "--version"], text=True
        )
    except Exception as e:
        ov_version_cmd = str(e)
    try:
        pkg_ver = version("open-vulnera")
    except PackageNotFoundError:
        pkg_ver = None
    ov_version = ov_version_cmd, pkg_ver
    return ov_version
...
def interpreter_info(vulnera):
    try:
        if vulnera.offline and vulnera.llm.api_base:
            try:
                curl = subprocess.check_output(f"curl {vulnera.llm.api_base}")
            except Exception as e:
                curl = str(e)
        else:
            curl = "Not local"
        ...
        Vision: {vulnera.llm.supports_vision}
        Model: {vulnera.llm.model}
        Function calling: {vulnera.llm.supports_functions}
        Context window: {vulnera.llm.context_window}
        Max tokens: {vulnera.llm.max_tokens}
        Computer API: {vulnera.computer.import_computer_api}

        Auto run: {vulnera.auto_run}
        API base: {vulnera.llm.api_base}
        Offline: {vulnera.offline}

        Curl output: {curl}

        # Messages

        System Message: {vulnera.system_message}

        """ + "\n\n".join(
            [str(m) for m in messages_to_display]
        )
    except:
        return "Error, couldn't get vulnera info"


def system_info(vulnera):
    ov_version = get_ov_version()
    print(
        f"""
        Python Version: {get_python_version()}
        Pip Version: {get_pip_version()}
        Open-vulnera Version: cmd: {ov_version[0]}, pkg: {ov_version[1]}
        OS Version and Architecture: {get_os_version()}
        CPU Info: {get_cpu_info()}
        RAM Info: {get_ram_info()}
        {interpreter_info(vulnera)}
    """
    )
