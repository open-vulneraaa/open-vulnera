# This is a websocket vulnera, TTS and STT disabled.
...
class AsyncInterpreter:
    def __init__(self, vulnera):
        ...
        # TTS
        # if self.vulnera.tts == "coqui":
        #     engine = CoquiEngine()
        # elif self.vulnera.tts == "openai":
        #     engine = OpenAIEngine()
        ...
        self._in_keyboard_write_block = (
            False  # Tracks whether vulnera is trying to use the keyboard
        )
    ...
    async def input(self, chunk):
        ...
        if isinstance(chunk, bytes):
            ...
            if "start" in chunk:
                ...
                self.vulnera.computer.terminate()
                # Stop any code execution... maybe we should make vulnera.stop()?
            elif "end" in chunk:
    ...
    async def run(self):
        """
        Runs Vulnera on the audio bytes submitted to the input. Will add streaming LMC chunks to the _output_queue.
        """
        # self.beeper.start()
        ...
        def generate(message):
            ...
            # vulnera.messages = self.active_chat_messages
            # print("🍀🍀🍀🍀GENERATING, using these messages: ", self.vulnera.messages)
            print("passing this in:", message)
            for chunk in self.vulnera.chat(message, display=False, stream=True):
                ...
                # Handle code blocks
                    ...
                    #     if self.vulnera.messages[-1]["content"].startswith(
                    #         "computer.keyboard.write("
                    #     ):
                    #         self.vulnera.messages[-1]["content"] = (
                    #             "dummy_variable = ("
                    #             + self.vulnera.messages[-1]["content"][
...
def server(vulnera, port=8000):  # Default port is 8000 if not specified
    async_interpreter = AsyncInterpreter(vulnera)
    ...
    async def settings(payload: Dict[str, Any]):
        for key, value in payload.items():
            print("Updating vulnera settings with the following:")
            print(key, value)
            if key == "llm" and isinstance(value, dict):
                for sub_key, sub_value in value.items():
                    setattr(async_interpreter.vulnera, sub_key, sub_value)
            else:
                setattr(async_interpreter.vulnera, key, value)
    ...
    vulnera.uvicorn_server = Server(config)
    vulnera.uvicorn_server.run()
