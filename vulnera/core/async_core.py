...
class AsyncInterpreter(OpenVulnera):
    ...
    async def input(self, chunk):
        """
        Accumulates LMC chunks onto vulnera.messages.
        When it hits an "end" flag, calls vulnera.respond().
        """

        if "start" in chunk:
            # If the user is starting something, the vulnera should stop.
            ...
            # If the user is done talking, the vulnera should respond.
        ...
        for attempt in range(5):  # 5 attempts
            try:
                ...
                for chunk_og in self._respond_and_store():
                    ...
                    if self.debug:
                        print("Vulnera produced this chunk:", chunk)
...
class Server:
    ...
    def run(self, host=None, port=None, retries=5):
        ...
        if self.host == "0.0.0.0":
            print(
                "Warning: Using host `0.0.0.0` will expose Open Vulnera over your local network."
            )
        ...
        #             raise ImportError(
        #                 str(e)
        #                 + """\n\nPlease ensure you have run `pip install "open-vulnera[server]"` to install server dependencies."""
        #             )
        #     except:
