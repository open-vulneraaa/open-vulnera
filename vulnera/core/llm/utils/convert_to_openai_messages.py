    ...
    for message in messages:
        ...
        if message["type"] == "message":
            ...
            if message["role"] == "user" and (
                ...
                or vulnera.always_apply_user_message_template
            ):
                # Only add the template for the last message?
                new_message["content"] = vulnera.user_message_template.replace(
            ...
            if function_calling:
                new_message["function_call"] = {
                    ...
                    # parsed_arguments isn't actually an OpenAI thing, it's an Open Vulnera thing.
                    # but it's soo useful!
                    # "parsed_arguments": {
            ...
            if function_calling:
                ...
                if vulnera.code_output_sender == "user":
                    if message["content"].strip() == "":
                        content = vulnera.empty_code_output_template
                    else:
                        content = vulnera.code_output_template.replace(
                            "{content}", message["content"]
                        )

                    new_message["role"] = "user"
                    new_message["content"] = content
                elif vulnera.code_output_sender == "assistant":
                    new_message["role"] = "assistant"
                    new_message["content"] = (
                        "\n```output\n" + message["content"] + "\n```"
                    )
