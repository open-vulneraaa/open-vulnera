...
class Llm:
    ...
    def __init__(self, vulnera):
        # Add the filter to the logger
        logger.addFilter(SuppressDebugFilter())

        # Store a reference to parent vulnera
        self.vulnera = vulnera
        ...
        self.vision_renderer = (
            self.vulnera.computer.vision.query
    ...
    def run(self, messages):
        ...
        starting with LMC messages in vulnera.messages, going to OpenAI compatible messages into the llm,
        ...
        # Removed legacy hosted "i" model mapping to Open Interpreter hosted API
        # REBRAND_REVIEW: removed OI hosted-model mapping and defaults
        ...
        if self.supports_vision:
            if self.vulnera.os:
                # Keep only the last two images if the vulnera is running in OS mode
                if len(image_messages) > 1:
                    for img_msg in image_messages[:-2]:
                        messages.remove(img_msg)
                        if self.vulnera.verbose:
            ...
            for img_msg in image_messages:
                if img_msg["format"] != "description":
                    ...
                    try:
                        ...
                        ocr = self.vulnera.computer.vision.ocr(lmc=img_msg)
                        ...
                        print(
                            "\nTo use local vision, run `pip install 'open-vulnera[local]'`.\n"
                        )
                        img_msg["format"] = "description"
                        img_msg["content"] = ""
        ...
        messages = convert_to_openai_messages(
            ...
            shrink_images=self.vulnera.shrink_images,
            vulnera=self.vulnera,
        )
        ...
        try:
            if self.context_window and self.max_tokens:
                ...
                try:
                    ...
                    if len(messages) == 1:
                        if self.vulnera.in_terminal_interface:
                            self.vulnera.display_message(
                                """
**We were unable to determine the context window of this model.** Defaulting to 8000.

If your model can handle more, run `vulnera --context_window {token limit} --max_tokens {max tokens per response}`.
...
Continuing...
        ...
        if hasattr(self.vulnera, "conversation_id"):
            params["conversation_id"] = self.vulnera.conversation_id
        ...
        if (
            self.vulnera.debug == True and False  # DISABLED
    ...
    def load(self):
        ...
        if self.model.startswith("ollama/"):
            ...
            if model_name not in names:
                self.vulnera.display_message(f"\nDownloading {model_name}...\n")
            ...
            self.max_tokens = 1
            self.vulnera.computer.ai.chat("ping")
            self.max_tokens = old_max_tokens

            self.vulnera.display_message("*Model loaded.*\n")
...
def fixed_litellm_completions(**params):
    ...
    # No special-case for the removed hosted 'i' model; always use default behavior
    ...
    for attempt in range(attempts):
        try:
            ...
            if (
                ...
                print(
                    "LiteLLM requires an API key. Trying again with a dummy API key. In the future, if this fixes it, please set a dummy API key to prevent this message. (e.g `vulnera --api_key x` or `self.api_key = 'x'`)"
                )
