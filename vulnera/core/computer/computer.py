...
class Computer:
    def __init__(self, vulnera):
        ...
        self.api_base = "https://api.openvulnera.com/v0"
        ...
        self.max_output = (
            self.vulnera.max_output
        )  # Should mirror vulnera.max_output
...
A python vulnera.computer module is ALREADY IMPORTED, and can be used for many tasks:

```python
{self._computer_tools}
```

Do not import the vulnera.computer module, or any of its sub-modules. They are already imported.

    """.strip()

    # Shortcut for vulnera.computer.terminal.languages
    @property
    def languages(self):
    ...
    def _get_all_computer_tools_signature_and_description(self):
        """
        This function returns a list of all the vulnera.computer tools that are available with their signature and description from the function docstrings.
        for example:
vulnera.computer.browser.search(query) # Searches the web for the specified query and returns the results.
vulnera.computer.calendar.create_event(title: str, start_date: datetime.datetime, end_date: datetime.datetime, location: str = "", notes: str = "", calendar: str = None) -> str # Creates a new calendar event in the default calendar with the given parameters using AppleScript.
        """
    ...
    def _extract_tool_info(self, tool):
        ...
        if tool.__class__.__name__ == "Browser":
            ...
            for name in dir(tool):
                ...
                if (
                    ...
                    full_signature = f"vulnera.computer.{tool.__class__.__name__.lower()}.{name}({param_str})"
        ...
        for name, method in inspect.getmembers(tool, predicate=inspect.ismethod):
            ...
            if not name.startswith("_") and not hasattr(method, "__wrapped__"):
                ...
                full_signature = (
                    f"vulnera.computer.{tool.__class__.__name__.lower()}.{name}({param_str})"
                )
                # Get the method description
    ...
    def run(self, *args, **kwargs):
        """
        Shortcut for vulnera.computer.terminal.run
        """
        return self.terminal.run(*args, **kwargs)

    def exec(self, code):
        """
        Shortcut for vulnera.computer.terminal.run("shell", code)
