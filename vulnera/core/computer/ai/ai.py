...
def fast_llm(llm, system_message, user_message):
    old_messages = llm.vulnera.messages
    old_system_message = llm.vulnera.system_message
    try:
        llm.vulnera.system_message = system_message
        llm.vulnera.messages = []
        response = llm.vulnera.chat(user_message)
    finally:
        llm.vulnera.messages = old_messages
        llm.vulnera.system_message = old_system_message
...
class Ai:
    ...
    def chat(self, text, base64=None):
        ...
        response = ""
        for chunk in self.computer.vulnera.llm.run(messages):
            if "content" in chunk:
                response += chunk.get("content")
        return response

        # Old way
        old_messages = self.computer.vulnera.llm.vulnera.messages
        old_system_message = self.computer.vulnera.llm.vulnera.system_message
        old_import_computer_api = self.computer.import_computer_api
        old_execution_instructions = (
            self.computer.vulnera.llm.execution_instructions
        )
        try:
            self.computer.vulnera.llm.vulnera.system_message = (
                "You are an AI assistant."
            )
            self.computer.vulnera.llm.vulnera.messages = []
            self.computer.import_computer_api = False
            self.computer.vulnera.llm.execution_instructions = ""

            response = self.computer.vulnera.llm.vulnera.chat(text)
        finally:
            self.computer.vulnera.llm.vulnera.messages = old_messages
            self.computer.vulnera.llm.vulnera.system_message = (
                old_system_message
            )
    ...
    def query(self, text, query, custom_reduce_query=None):
        ...
        chunks = split_into_chunks(
            text, chunk_size, self.computer.vulnera.llm, overlap
        )

        # (Map) Query each chunk
        responses = query_map_chunks(chunks, self.computer.vulnera.llm, query)

        # (Reduce) Compress the responses
        response = query_reduce_chunks(
            responses, self.computer.vulnera.llm, chunk_size, custom_reduce_query
        )
