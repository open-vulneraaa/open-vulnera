...
""".strip()
        ...
        # Disable color outputs in the terminal, which don't look good in Open Vulnera and aren't useful
        # code = """
    ...
    def _execute_code(self, code, message_queue):
        def iopub_message_listener():
            ...
            while True:
                ...
                # For async usage
                if (
                    hasattr(self.computer.vulnera, "stop_event")
                    and self.computer.vulnera.stop_event.is_set()
                ):
                    self.km.interrupt_kernel()
                ...
                try:
                    ...
                    if (
                        ...
                        params = {
                            "messages": messages,
                            "model": self.computer.vulnera.llm.model,
                            "stream": True,
                            "temperature": 0,
                        }
                        if self.computer.vulnera.llm.api_key:
                            params["api_key"] = self.computer.vulnera.llm.api_key
