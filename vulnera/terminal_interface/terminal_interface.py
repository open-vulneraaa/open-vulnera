...
def terminal_interface(vulnera, message):
    ...
    if (
        ...
        if vulnera.safe_mode == "ask" or vulnera.safe_mode == "auto":
            if not check_for_package("semgrep"):
                interpreter_intro_message.append(
                    f"**Safe Mode**: {vulnera.safe_mode}\n\n>Note: **Safe Mode** requires `semgrep` (`pip install semgrep`)"
                )
        else:
            interpreter_intro_message.append("Use `vulnera -y` to bypass this.")

        if (
            not vulnera.plain_text_display
    ...
    while True:
        if interactive:
            if (
                len(vulnera.messages) == 1
                and vulnera.messages[-1]["role"] == "user"
                and vulnera.messages[-1]["type"] == "message"
            ):
                # They passed in a message already, probably via "i {command}"!
                message = vulnera.messages[-1]["content"]
                vulnera.messages = vulnera.messages[:-1]
            else:
                ### This is the primary input for Open Vulnera.
                try:
                    message = (
                        cli_input("> ").strip()
                        if vulnera.multi_line
                        else input("> ").strip()
                    )
        ...
        if isinstance(message, str):
            ...
            if message.startswith("%") and interactive:
                handle_magic_command(vulnera, message)
                continue

            # Many users do this
            if message.strip() == "vulnera --local":
                print("Please exit this conversation, then run `vulnera --local`.")
                continue
            if message.strip() == "pip install --upgrade open-vulnera":
                print(
                    "Please exit this conversation, then run `pip install --upgrade open-vulnera`."
                )
                continue

            if (
                vulnera.llm.supports_vision
                or vulnera.llm.vision_renderer != None
            ):
                ...
                if image_path:
                    # Add the text vulnera's message history
                    ...
                    # Pass in the image to vulnera in a moment
                    message = {
                        "role": "user",
                        "type": "image",
                        "format": "path",
        ...
        try:
            for chunk in vulnera.chat(message, display=False, stream=True):
                yield chunk
                ...
                if chunk["type"] == "confirmation":
                    if not vulnera.auto_run:
                        # Open Vulnera is about to execute code. The user wants to approve this

                        # End the active code block so you can run input() below it
                        if active_block and not vulnera.plain_text_display:
                        ...
                        if not vulnera.safe_mode == "off":
                            if vulnera.safe_mode == "auto":
                                should_scan_code = True
                            elif vulnera.safe_mode == "ask":
                                response = input(
                        ...
                        if should_scan_code:
                            scan_code(code, language, vulnera)

                        if vulnera.plain_text_display:
                            response = input(
                        ...
                        if response.strip().lower() == "y":
                            ...
                            active_block = CodeBlock(vulnera)
                            ...
                            vulnera.messages[-1]["content"] = code  # Give it code
                ...
                if chunk["type"] == "message":
                    ...
                    if "end" in chunk and vulnera.os:
                        last_message = vulnera.messages[-1]["content"]
                        ...
                        vulnera.computer.os.notify(sanitized_message)

                        # Speak message aloud
                        if platform.system() == "Darwin" and vulnera.speak_messages:
                            if voice_subprocess:
                ...
                if (
                    ...
                    if (vulnera.os == True) and (vulnera.verbose == False):
                    ...
                    if (
                        vulnera.messages[-1].get("format") != "output"
                        or vulnera.messages[-1]["role"] != "computer"
                        or vulnera.messages[-1]["type"] != "console"
                    ):
                        ...
                        vulnera.messages[-1]["content"] += (
                            "\n" + extra_computer_output
                        )
                        vulnera.messages[-1]["content"] = vulnera.messages[-1][
                            "content"
                        ].strip()
                    ...
                    if "format" in chunk and chunk["format"] == "active_line":
                        ...
                        if vulnera.os and active_block.active_line != None:
                            ...
                            if action.startswith("computer"):
                                ...
                                if description:
                                    vulnera.computer.os.notify(description)
