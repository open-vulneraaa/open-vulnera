...
from ..utils.ov_dir import ov_dir
from .historical_profiles import historical_profiles

profile_dir = os.path.join(ov_dir, "profiles")
user_default_profile_path = os.path.join(profile_dir, "default.yaml")

here = os.path.abspath(os.path.dirname(__file__))
ov_default_profiles_path = os.path.join(here, "defaults")
default_profiles_paths = glob.glob(os.path.join(ov_default_profiles_path, "*"))
default_profiles_names = [os.path.basename(path) for path in default_profiles_paths]

# Constant to hold the version number (synced to pyproject.toml)
try:
...
def profile(vulnera, filename_or_url):
    ...
    return apply_profile(vulnera, profile, profile_path)


def get_profile(filename_or_url, profile_path):
    # i.com/ is a shortcut for openvulnera.com/profiles/
    ...
    for shortcut in shortcuts:
        if filename_or_url.startswith(shortcut):
            filename_or_url = filename_or_url.replace(
                shortcut, "https://openvulnera.com/profiles/"
            )
    ...
    if os.path.exists(profile_path):
        with open(profile_path, "r", encoding="utf-8") as file:
            if extension == ".py":
                ...
                # Remove `from vulnera import vulnera` and `vulnera = OpenVulnera()`, because we handle that before the script
    ...
    response.raise_for_status()
    if extension == ".py":
        return {"start_script": response.text, "version": OV_VERSION}
    elif extension == ".json":
...
class RemoveInterpreter(ast.NodeTransformer):
    """Remove `from vulnera import vulnera` and `vulnera = OpenVulnera()`"""

    def visit_ImportFrom(self, node):
        if node.module == "vulnera":
            for alias in node.names:
                if alias.name == "vulnera":
                    return None
        return node
    ...
    def visit_Assign(self, node):
        if (
            ...
            and node.targets[0].id == "vulnera"
            and isinstance(node.value, ast.Call)
            and isinstance(node.value.func, ast.Name)
            and node.value.func.id == "OpenVulnera"
        ):
            return None  # None will remove the node from the AST
        return node  # return node otherwise to keep it in the AST


def apply_profile(vulnera, profile, profile_path):
    if "start_script" in profile:
        scope = {"vulnera": vulnera}
        exec(profile["start_script"], scope, scope)

    if (
        "version" not in profile or profile["version"] != OV_VERSION
        ...
        if message.lower() == "y":
            ...
            if profile_path.endswith("default.yaml"):
                with open(profile_path, "r") as file:
                    text = file.read()
                text = text.replace(
                    "version: " + str(profile["version"]), f"version: {OV_VERSION}"
                )
    ...
    if "computer" in profile and "languages" in profile["computer"]:
        ...
        vulnera.computer.languages = [
            i
            for i in vulnera.computer.languages
            if i.name.lower() in [l.lower() for l in profile["computer"]["languages"]]
        ]
        del profile["computer"]["languages"]

    apply_profile_to_object(vulnera, profile)
...
def migrate_profile(old_path, new_path):
    ...
    old_system_messages = [
        """You are Open Vulnera, a world-class programmer that can.complete any goal by executing code.
...
You are capable of **any** task.""",
        """You are Open Vulnera, a world-class programmer that can.complete
...
# safe_mode: "off"  # The safety mode (see https://github.com/open-vulnera/open-vulnera/blob/master/docs/SAFE_MODE.md)
...
# All options: https://github.com/open-vulnera/open-vulnera/tree/master/docs/settings
...
def open_storage_dir(directory):
    dir = os.path.join(ov_dir, directory)
...
def get_default_profile(specific_default_profile):
    for default_yaml_file in default_profiles_paths:
        ...
        profile_path = os.path.join(ov_default_profiles_path, filename)
...
def determine_user_version():
    # Pre 0.2.0 directory
    old_dir_pre_020 = platformdirs.user_config_dir("Open Vulnera")
    # 0.2.0 directory
    old_dir_020 = platformdirs.user_config_dir("Open Vulnera Terminal")

    if os.path.exists(ov_dir) and os.listdir(ov_dir):
        # Check if the default.yaml profile exists and has a version key
        default_profile_path = os.path.join(ov_dir, "profiles", "default.yaml")
...
def migrate_app_directory(old_dir, new_dir, profile_dir):
    ...
    # After all migrations have taken place, every yaml file should have a version listed. Sometimes, if the user does not have a default.yaml file from 0.2.0, it will not add the version to the file, causing the migration message to show every time vulnera is launched. This code loops through all yaml files post migration, and ensures they have a version number, to prevent the migration message from showing.
...
def migrate_user_app_directory():
    ...
    if user_version == "pre_0.2.0":
        old_dir = platformdirs.user_config_dir("Open Vulnera")
        migrate_app_directory(old_dir, ov_dir, profile_dir)

    elif user_version == "0.2.0":
        old_dir = platformdirs.user_config_dir("Open Vulnera Terminal")
        migrate_app_directory(old_dir, ov_dir, profile_dir)
