    ...
    if user_input.lower() != "y":
        ...
        for pip_combo in [
            ...
            if pip_combo[1] == "quotes":
                command = f'{pip_combo[0]} install "open-vulnera[os]"'
            else:
                command = f"{pip_combo[0]} install open-vulnera[os]"

            vulnera.computer.run("shell", command, display=True)
...
vulnera.display_message("> `OS Control` enabled")
...
if not vulnera.auto_run:
    ...
    vulnera.display_message(screen_recording_message)
    print("")

# # FOR TESTING ONLY
# # Install Open Vulnera from GitHub
# for chunk in vulnera.computer.run(
#     "shell",
#     "pip install git+https://github.com/open-vulnera/open-vulnera.git",
# ):
...
vulnera.display_message(
    "**Warning:** In this mode, Open Vulnera will not require approval before performing actions. Be ready to close your terminal."
)
