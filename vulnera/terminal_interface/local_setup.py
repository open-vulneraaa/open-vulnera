...
def local_setup(vulnera, provider=None, model=None):
    def download_model(models_dir, models, vulnera):
        ...
        try:
            ...
            if filtered_models:
                ...
                vulnera.display_message(
                    "To view or delete downloaded local models, run `vulnera --local_models`\n\n"
                )
    ...
    vulnera.display_message(
        "\n**Open Vulnera** supports multiple local model providers.\n"
    )
    ...
    if selected_model == "LM Studio":
        vulnera.display_message(
            """
    To use Open Vulnera with **LM Studio**, you will need to run **LM Studio** in the background.
    ...
    """
        ...
        vulnera.llm.api_base = "http://localhost:1234/v1"
        vulnera.llm.api_key = "dummy"
        ...
        try:
            ...
            vulnera.llm.model = f"ollama/{model}"

            # Send a ping, which will actually load the model

            old_max_tokens = vulnera.llm.max_tokens
            old_context_window = vulnera.llm.context_window
            vulnera.llm.max_tokens = 1
            vulnera.llm.context_window = 100

            vulnera.computer.ai.chat("ping")

            vulnera.llm.max_tokens = old_max_tokens
            vulnera.llm.context_window = old_context_window

            vulnera.display_message(f"> Model set to `{model}`")
        ...
        vulnera.display_message(
            """
    To use Open Vulnera with **Jan**, you will need to run **Jan** in the background.
    ...
    """
        ...
        vulnera.llm.api_base = "http://localhost:1337/v1"
        # time.sleep(1)

        # Send a GET request to the Jan API to get the list of models
        response = requests.get(f"{vulnera.llm.api_base}/models")
        ...
        vulnera.llm.model = jan_model_name
        vulnera.llm.api_key = "dummy"
        ...
        if platform.system() == "Darwin":  # Check if the system is MacOS
            ...
            if result.returncode != 0:
                vulnera.display_message(
                    "To use Llamafile, Open Vulnera requires Mac users to have Xcode installed. You can install Xcode from https://developer.apple.com/xcode/ .\n\nAlternatively, you can use `LM Studio`, `Jan.ai`, or `Ollama` to manage local language models. Learn more at https://github.com/open-vulnera/open-vulnera/tree/master/docs/guides/running-locally ."
                )
                time.sleep(3)
                raise Exception(
        ...
        models_dir = os.path.join(vulnera.get_ov_dir(), "models")
        ...
        if not models:
            ...
            model_path = download_model(models_dir, models, vulnera)
        else:
        ...
        # Set flags for Llamafile to work with vulnera
        vulnera.llm.model = "openai/local"
        vulnera.llm.api_key = "dummy"
        vulnera.llm.temperature = 0
        vulnera.llm.api_base = "http://localhost:8080/v1"
        vulnera.llm.supports_functions = False

        model_name = model_path.split("/")[-1]
        vulnera.display_message(f"> Model set to `{model_name}`")
