...
Open Vulnera is [changing](https://changes.openvulnera.com/log/the-new.computer-update). This guide will help you migrate your application to `0.2.0`, also called the _New Computer Update_ (NCU), the latest major version of Open Vulnera.

## A New Start

To start using Open Vulnera in Python, we now use a standard **class instantiation** format:

```python
# From the module `vulnera`, import the class `OpenVulnera`
from vulnera import OpenVulnera

# Create an instance of `OpenVulnera` to use it
agent = OpenVulnera()
agent.chat()
```

For convenience, we also provide an instance of `vulnera`, which you can import from the module (also called `vulnera`):

```python
 # From the module `vulnera`, import the included instance of `OpenVulnera`
...
All stateless LLM attributes have been moved to `vulnera.llm`:

- `vulnera.model` → `vulnera.llm.model`
- `vulnera.api_key` → `vulnera.llm.api_key`
- `vulnera.llm_supports_vision` → `vulnera.llm.supports_vision`
- `vulnera.supports_function_calling` → `vulnera.llm.supports_functions`
- `vulnera.max_tokens` → `vulnera.llm.max_tokens`
- `vulnera.context_window` → `vulnera.llm.context_window`
- `vulnera.temperature` → `vulnera.llm.temperature`
- `vulnera.api_version` → `vulnera.llm.api_version`
- `vulnera.api_base` → `vulnera.llm.api_base`

This is reflected **1)** in Python applications using Open Vulnera and **2)** in your profile for Open Vulnera's terminal interface, which can be edited via `vulnera --profiles`.

This is reflected in both Python applications using Open Vulnera and in your profile for Open Vulnera's terminal interface.
...
- This allows you to pass the full `messages` list into Open Vulnera as `vulnera.messages = message_list`.
...
- If you want your application to run the code instead of Open Vulnera, then your app will act as the .computer`. This means breaking from the stream once Open Vulnera emits a confirmation chunk (`{'role': .computer', 'type': 'confirmation' ...}`) to prevent it from running the code. When you run code, grab the message history via `messages = vulnera.messages`, then simply mimic the .computer` format above by appending new `{'role': .computer' ...}` messages, then run `vulnera.chat(messages)`.
- Open Vulnera is designed to stop code execution when the stream is disconnected. Use this to your advantage to add a "Stop" button to the UI.
...
// Function to send a POST request to Open Vulnera
async function sendRequest() {
  // Temporary message to hold the message that is being processed
  try {
    // Define parameters for the POST request, add at least the full messages array, but you may also consider adding any other Open Vulnera parameters here, like auto_run, local, etc.
    const params = {
      messages,
    };
    ...
    if (!interpreterCall.ok) {
      console.error("Vulnera didn't respond with 200 OK");
      return;
    }
    ...
    while (true) {
      ...
      // If isGenerating is set to false, cancel the reader and break the loop. This will halt the execution of the code run by Open Vulnera as well
      if (!isGenerating) {
        await reader.cancel();
        controller.abort();
        break;
