...
### Message Format
Open Vulnera uses an extended version of OpenAI's message format called [LMC messages](https://docs.openvulnera.com/protocols/lmc-messages) that allow for rich, multi-part messages. **Messages must be sent between start and end flags.** Here's the basic structure:
...
### Modifying Settings
To change server settings, send a POST request to `http://localhost:8000/settings`. The payload should conform to [the vulnera object's settings](https://docs.openvulnera.com/settings/all-settings).
...
```bash
docker build -t open-vulnera .
```

Then, run the container:

```bash
docker run -p 8000:8000 open-vulnera
```
...
4. Use the AsyncVulnera class when working with the server in Python to ensure compatibility with asynchronous operations.
