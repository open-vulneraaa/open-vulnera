...
Open Vulnera is working on providing an experimental safety toolkit to help you feel more confident running the code generated by Open Vulnera.

Install Open Vulnera with the safety toolkit dependencies as part of the bundle:
...
You can enable safe mode by passing the `--safe` flag when invoking `vulnera` or by configuring `safe_mode` in your [config file](https://github.com/open-vulnera/open-vulnera#configuration).
...
- [Execute code in containers](https://github.com/open-vulnera/open-vulnera/pull/459)

## Tips & Tricks

You can adjust the `system_message` in your [config file](https://github.com/open-vulnera/open-vulnera#configuration) to include instructions for the model to scan packages with [`guarddog`]() before installing them.
