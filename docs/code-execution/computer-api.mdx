...
The following functions are designed for language models to use in Open Vulnera, currently only supported in [OS Mode](/guides/os-mode/).
...
```python
x, y = vulnera.computer.display.center()
```
...
```python
vulnera.computer.keyboard.hotkey(" ", "command")
```
...
```python
vulnera.computer.keyboard.write("hello")
```
...
vulnera.computer.mouse.click(x=100, y=100)

# Click on text on the screen
vulnera.computer.mouse.click("Onscreen Text")

# Click on a gear icon
vulnera.computer.mouse.click(icon="gear icon")
```
...
```python
# Click on coordinates
vulnera.computer.mouse.move(x=100, y=100)

# Click on text on the screen
vulnera.computer.mouse.move("Onscreen Text")

# Click on a gear icon
vulnera.computer.mouse.move(icon="gear icon")
```
...
```python
# Scroll Down
vulnera.computer.mouse.scroll(-10)
...
```python
vulnera.computer.clipboard.view()
```
...
```python
vulnera.computer.os.get_selected_text()
```
...
```python
vulnera.computer.mail.get(number=10, unread=True)
```
...
```python
vulnera.computer.mail.send("john@email.com", "Subject", "Body", ["path/to/attachment.pdf", "path/to/attachment2.pdf"])
```
...
```python
vulnera.computer.mail.unread_count()
```
...
```python
vulnera.computer.sms.send("2068675309", "Hello from Open Vulnera!")
```
...
```python
vulnera.computer.contacts.get_phone_number("John Doe")
```
...
```python
vulnera.computer.calendar.get_events(start_date=datetime, end_date=datetime)
```
...
```python
vulnera.computer.calendar.create_event(title="Title", start_date=datetime, end_date=datetime, location="Location", notes="Notes", calendar="Work")
```
...
```python
vulnera.computer.calendar.delete_event(event_title="Title", start_date=datetime, calendar="Work")
```
