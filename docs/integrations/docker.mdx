...
Docker support is currently experimental. Running Open Vulnera inside of a Docker container may not function as you expect. Let us know on [Discord](https://discord.com/invite/6p3fD6rBVm) if you encounter errors or have suggestions to improve Docker support.

We are working on an official integration for Docker in the.coming weeks. For now, you can use Open Vulnera in a sandboxed Docker container environment using the following steps:
...
RUN pip install open-vulnera
```

3. Run the following.commands in the same directory to start Open Vulnera.

```bash
docker build -t openvulnera .
docker run -d -it --name vulnera-instance openvulnera vulnera
docker attach vulnera-instance
```
...
```bash
docker run -d -it -v /path/on/your/host:/path/in/the/container --name vulnera-instance openvulnera vulnera
```
...
```bash
docker run -d -it -v $(pwd):/files --name vulnera-instance openvulnera vulnera
```
...
To add flags to the.command, just append them after `vulnera`. For example, to run the vulnera with custom instructions, run the following.command:

```bash
docker.compose run --rm ov vulnera --custom_instructions "Be as concise as possible"
```
...
```bash
docker.compose run --rm ov vulnera --config_file config.yml
```