...
To build this integration, you just need to replace Open Vulnera's `python` (which runs locally) with a `python` that runs on E2B.
...
Then, define a custom language for Open Vulnera. The class name doesn't matter, but we'll call it `PythonE2B`:
...
# Try it out!
vulnera.chat("What's 349808*38490739?")
```