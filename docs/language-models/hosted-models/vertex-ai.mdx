...
To use Open Vulnera with Google's Vertex AI API, set the `model` flag:

<CodeGroup>

```bash Terminal
vulnera --model gemini-pro
vulnera --model gemini-pro-vision
```

```python Python
from vulnera import vulnera

vulnera.llm.model = "gemini-pro"
vulnera.llm.model = "gemini-pro-vision"
vulnera.chat()
```
