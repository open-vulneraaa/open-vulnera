...
To run Open Vulnera with Jan.ai, follow these steps:
...
5. Now we fire up Open Vulnera with this custom model. Either run `vulnera --local` in the terminal to set it up interactively, or run this command, but replace `<model_id>` with the id of the model you downloaded:

<CodeGroup>

```bash Terminal
vulnera --api_base http://localhost:1337/v1  --model <model_id>
```
...
```bash Terminal
vulnera --api_base http://localhost:1337/v1  --model <model_id> --context_window 5000
```

```python Python
from vulnera import vulnera

vulnera.context_window = 5000
```
