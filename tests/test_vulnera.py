...
# some tests depend on optional packages; skip if unavailable
import pytest
pytest.importorskip("fastapi")
pytest.importorskip("websockets")

# ensure tests don't fail due to missing API key
...
# Avoid any real LLM network requests during tests by stubbing run method

def _dummy_run(messages, *args, **kwargs):
    # yield nothing by default
    if False:
        yield

vulnera.llm.run = _dummy_run
...
def test_hallucinations():
    ...
    for chunk in vulnera._respond_and_store():
...
def test_server():
    ...
    async def test_fastapi_server():
        ...
        async with websockets.connect("ws://localhost:8000/") as websocket:
            ...
            response = vulnera.computer.ai.chat(
                str(messages)
            ...
            #             "type": "image",
            #             "format": "path",
            #             "content": "/Users/killianlucas/Documents/GitHub/open-vulnera/screen.png",
            #         }
            #     )
            # )
...
def test_pytes():
    ...
    if files_on_desktop:
        ...
        ocr = vulnera.computer.vision.ocr(path=first_file_path)
        print(ocr)
        print("what")
    else:
...
def test_ai_chat():
    print(vulnera.computer.ai.chat("hi"))
...
@pytest.mark.skip(reason="Requires open-vulnera[local]")
def test_localos():
    vulnera.computer.emit_images = False
    vulnera.computer.view()
...
def test_point():
    # vulnera.computer.debug = True
    vulnera.computer.mouse.move(icon="gear")
    vulnera.computer.mouse.move(icon="refresh")
    vulnera.computer.mouse.move(icon="play")
    vulnera.computer.mouse.move(icon="magnifying glass")
    vulnera.computer.mouse.move("Spaces:")
    assert False
...
def test_skills():
    ...
    # vulnera.computer.skills.path = skills_path
    print(vulnera.computer.skills.path)
    if os.path.exists(vulnera.computer.skills.path):
        for file in os.listdir(vulnera.computer.skills.path):
            os.remove(os.path.join(vulnera.computer.skills.path, file))
    print("Path: ", vulnera.computer.skills.path)
    print("Files in the path: ")
    vulnera.computer.run("python", "def testing_skilsl():\n    print('hi')")
    for file in os.listdir(vulnera.computer.skills.path):
        print(file)
    vulnera.computer.run("python", "def testing_skill():\n    print('hi')")
    print("Files in the path: ")
    for file in os.listdir(vulnera.computer.skills.path):
        print(file)

    try:
        skills = vulnera.computer.skills.search(query)
    except ImportError:
...
def test_browser():
    vulnera.computer.api_base = "http://0.0.0.0:80/v0"
    print(
        vulnera.computer.browser.search("When's the next Dune showing in Seattle?")
    )
    assert False
...
def test_display_api():
    ...
    # vulnera.computer.display.find_text("submit")
    # assert False
    ...
    for icon in icons:
        if icon.endswith("icon icon"):
            say("click the " + icon)
            vulnera.computer.mouse.move(icon=icon.replace("icon icon", "icon"))
        elif icon.endswith("icon"):
            say("click the " + icon)
            vulnera.computer.mouse.move(icon=icon.replace(" icon", ""))
        elif icon.endswith("text"):
            say("click " + icon)
            vulnera.computer.mouse.move(icon.replace(" text", ""))
        else:
            say("click " + icon)
            vulnera.computer.mouse.move(icon=icon)

    # vulnera.computer.mouse.move(icon="caution")
    # vulnera.computer.mouse.move(icon="bluetooth")
    # vulnera.computer.mouse.move(icon="gear")
    # vulnera.computer.mouse.move(icon="play button")
    # vulnera.computer.mouse.move(icon="code icon with '>_' in it")
...
def test_websocket_server():
    ...
    server_thread = threading.Thread(target=vulnera.server)
    server_thread.start()
    ...
    ws.send(
        "Hello, vulnera! What operating system are you on? Also, what time is it in Seattle?"
    )
...
def test_i():
    ...
    data = "Hello, vulnera! What operating system are you on? Also, what time is it in Seattle?"
...
def test_async():
    vulnera.chat("Hello!", blocking=False)
    print(vulnera.wait())
...
@pytest.mark.skip(reason="Computer with display only + no way to fail test")
def test_notify():
    vulnera.computer.os.notify("Hello")
    assert False


@pytest.mark.skip(reason="Computer with display only + no way to fail test")
def test_get_text():
    print(vulnera.computer.display.get_text_as_list_of_lists())
    assert False
...
def test_keyboard():
    time.sleep(2)
    vulnera.computer.keyboard.write("Hello " * 50 + "\n" + "hi" * 50)
    assert False
...
def test_get_selected_text():
    ...
    time.sleep(1)
    text = vulnera.computer.os.get_selected_text()
    print(text)
    assert False
...
def test_display_verbose():
    ...
    vulnera.computer.mouse.move(x=500, y=500)
    assert False
...
def test_long_message():
    ...
    vulnera.llm.context_window = 300
    vulnera.chat(messages)
    assert len(vulnera.messages) > 1
    assert "A" in vulnera.messages[-1]["content"]
...
def test_spotlight():
    vulnera.computer.keyboard.hotkey("command", "space")
...
def test_y_flag_preserves_system_prompt_text():
    from vulnera import init_vulnera
    v = init_vulnera()
    v.auto_run = True

    # ensure template is expanded and not sent as literal placeholders
    from vulnera.core.default_system_message import default_system_message
    assert "{_os}" not in default_system_message
    assert "{_python_ver}" not in default_system_message

    # shield: no direct system role should be in final conversation store
    v.llm.run = lambda messages: iter([{"role":"assistant","type":"message","content":"test"}])
    v.chat("who are you?", display=False, stream=True)
    assert all(msg.get("role") != "system" for msg in v.messages)
...
def test_multiple_instances():
    vulnera.system_message = "i"
    agent_1 = OpenVulnera()
    agent_1.system_message = "<3"
    agent_2 = OpenVulnera()
    agent_2.system_message = "u"

    assert vulnera.system_message == "i"
...
def test_hello_world():
    ...
    messages = vulnera.chat(hello_world_message)
...
def test_math():
    ...
    print("loading")
    messages = vulnera.chat(order_of_operations_message)
    print("done")
...
with open('numbers.txt', 'a+') as f:                                                                                                        
    ...
    for chunk in vulnera.computer.run("python", code, stream=True, display=True):
        print(chunk)
...
def test_write_to_file():
    ...
    vulnera.messages = []  # Just reset message history, nothing else for this test
    messages = vulnera.chat(
...
def test_reset():
    # make sure that vulnera.reset() clears out the messages Array
    assert vulnera.messages == []


def test_token_counter():
    system_tokens = count_tokens(
        text=vulnera.system_message, model=vulnera.llm.model
    )

    prompt = "How many tokens is this?"

    prompt_tokens = count_tokens(text=prompt, model=vulnera.llm.model)

    messages = [
        {"role": "system", "message": vulnera.system_message}
    ] + vulnera.messages

    system_token_test = count_messages_tokens(
        messages=messages, model=vulnera.llm.model
    )
